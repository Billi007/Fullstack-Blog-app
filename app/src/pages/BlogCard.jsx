...
const BlogCard = ({ blog, currentUser, blogId }) => {
  ...
  return (
    <>
      <div className="relative max-w-md bg-white dark:bg-black rounded-sm 
      shadow-xl hover:shadow-2xl transition-transform duration-300 transform hover:-translate-y-2 group">
        <div>
          <img
            className="rounded-t-sm w-full max-h-60 object-cover"
        ...
        <div className="content px-5">
          <h3 className="text-xl font-semibold mt-3 cursor-pointer dark:hover:text-blue-300 hover:text-blue-700 hover:underline">
            <Link to={`/blog/${blog._id}`}>{blog?.title}</Link>
          </h3>
          <p className="truncate-2-lines text-[15px] text-gray-700 dark:text-gray-400  my-5">
            {blog?.description}
          </p>
